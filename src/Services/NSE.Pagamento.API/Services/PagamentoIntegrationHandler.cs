...
namespace NSE.Pagamentos.API.Services
{
    public class PagamentoIntegrationHandler : BackgroundService
    {
        ...
        private void SetSubscribers()
        {
            ...
            _bus.SubscribeAsync<PedidoBaixadoEstoqueIntegrationEvent>("PedidoCancelado", async request =>
        ...
        private async Task<ResponseMessage> AutorizarPagamento(PedidoIniciadoIntegrationEvent message)
        {
            ...
            {
                var pagamentoService = scope.ServiceProvider.GetRequiredService<IPagamentosService>();

                response = await pagamentoService.AutorizarPagamento(ParaPagamento(message));
            }
        ...
        private async Task CancelarPagamento(PedidoCanceladoIntegrationEvent message)
        {
            using(var scope = _serviceProvider.CreateScope())
            {
                var service = scope.ServiceProvider.GetRequiredService<IPagamentosService>();

                var response = await service.CancelarPagamento(message.PedidoId);
        ...
        private async Task CapturarPagamento(PedidoBaixadoEstoqueIntegrationEvent message)
        {
            using (var scope = _serviceProvider.CreateScope())
            {
                ...
                var response = await service.CapturarPagamento(message.PedidoId);
        ...
        private Pagamento ParaPagamento(PedidoIniciadoIntegrationEvent message)
        {
